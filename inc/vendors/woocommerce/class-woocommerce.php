...
if ( ! class_exists( 'osf_WooCommerce' ) ) :
	...
	class osf_WooCommerce {
		...
		public function mobile_handheld_footer_bar() {
			$links = array(
				'my-account' => '<a class="my-accrount-footer" href="' . esc_url( get_permalink( get_option( 'woocommerce_myaccount_page_id' ) ) ) . '">' . esc_attr__( 'My Account', 'lexrider-core' ) . '</a>',
				'search'     => '<a class="search-footer" href="">' . esc_attr__( 'Search', 'lexrider-core' ) . '</a><div class="site-search">' . get_search_form( false ) . '</div>',
				'cart'       => ' <a class="footer-cart-contents" href="' . esc_url( wc_get_cart_url() ) . '" 
                title="' . esc_attr__( 'View your shopping cart', 'lexrider-core' ) . '">  
 <span class="count">' . wp_kses_data( WC()->cart->get_cart_contents_count() ) . '</span>
		...
		public function label_tooltip() {
			...
			echo '<div id="osf-woocommerce-cart">' . esc_html__( 'Add to cart', 'lexrider-core' ) . '</div>';
			echo '</div>';
		}
		...
		public function added_to_cart_template() {
			$text = esc_html__( 'has been added to your cart', 'lexrider-core' );
			echo <<<HTML
...
HTML;
		...
		public function widgets_init() {
			register_sidebar( array(
				'name'          => __( 'WooCommerce Shop', 'lexrider-core' ),
				'id'            => 'sidebar-woocommerce-shop',
				'description'   => __( 'Add widgets here to appear in your sidebar on blog posts and archive pages.', 'lexrider-core' ),
			...
			register_sidebar( array(
				'name'          => __( 'WooCommerce Detail', 'lexrider-core' ),
			...
			register_sidebar( array(
				'name'          => __( 'WooCommerce Shop Filters', 'lexrider-core' ),
		...
		public function render_woocommerce_shop_canvas() {
			...
			if ( is_active_sidebar( 'sidebar-woocommerce-shop-filters' ) ) {
				echo '<div id="opal-canvas-filter" class="opal-canvas-filter ' . $position . '"><span class="filter-close">' . esc_html__( 'CLOSE', 'lexrider-core' ) . '</span><div class="opal-canvas-filter-wrap">';
		...
		public function render_button_shop_canvas() {
			...
			if ( is_active_sidebar( 'sidebar-woocommerce-shop-filters' ) ) {
				echo '<button class="filter-toggle" aria-expanded="false"><span class="filter-icon"></span>' . esc_html__( 'Filter', 'lexrider-core' ) . '</button>';
			}
		}
			...
			if ( ! empty( $query ) ) {
				...
				if ( is_object( $product_object ) ) {
					...
					if ( ! empty( $product_sku ) ) {
						$product_sku_display = ' - ' . __( 'Sku', 'lexrider-core' ) . ': ' . $product_sku;
					}

					$product_title_display = '';
					if ( ! empty( $product_title ) ) {
						$product_title_display = ' - ' . __( 'Title', 'lexrider-core' ) . ': ' . $product_title;
					}

					$product_id_display = __( 'Id', 'lexrider-core' ) . ': ' . $product_id;
		...
		public function productCategoryAutocompleteRender( $query ) {
			...
			$term_slug_display = '';
			if ( ! empty( $term_slug ) ) {
				$term_slug_display = ' - ' . __( 'Sku', 'lexrider-core' ) . ': ' . $term_slug;
			}

			$term_title_display = '';
			if ( ! empty( $term_title ) ) {
				$term_title_display = ' - ' . __( 'Title', 'lexrider-core' ) . ': ' . $term_title;
			}

			$term_id_display = __( 'Id', 'lexrider-core' ) . ': ' . $term_id;
		...
		public function taxonomy_metaboxes() {
			...
			$cmb_term = new_cmb2_box( array(
				'id'           => 'product_cat',
				'title'        => __( 'Product Metabox', 'lexrider-core' ), // Doesn't output for term boxes
		...
		public function product_video_custom_field() {
			...
			$cmb    = new_cmb2_box( array(
				...
				'title'        => esc_html__( 'Product Video Config', 'lexrider-core' ),
			...
			$cmb->add_field( array(
				'name' => __( 'Product video', 'lexrider-core' ),
				'desc' => __( 'Supports video from youtube and vimeo.', 'lexrider-core' ),
				'id'   => $prefix . 'video',
				'type' => 'oembed',
			) );

			$cmb->add_field( array(
				'name'         => __( 'Video Thumbnail', 'lexrider-core' ),
		...
		public function woocommerce_scripts() {
			...
			wp_enqueue_script( 'osf-woocommerce-main', trailingslashit( LEXRIDER_CORE_PLUGIN_URL ) . 'assets/js/woocommerce/main.js', array(
				'jquery',
				'wp-util'
			), false, true );

			if ( is_product() ) {
				wp_enqueue_script( 'osf-woocommerce-single', trailingslashit( LEXRIDER_CORE_PLUGIN_URL ) . 'assets/js/woocommerce/single.js', array(), false, true );
				if ( get_theme_mod( 'osf_woocommerce_single_product_tab_style', 'tab' ) == 'accordion' ) {
					wp_enqueue_script( 'osf-woocommerce-single-accordion', trailingslashit( LEXRIDER_CORE_PLUGIN_URL ) . 'assets/js/woocommerce/woocommerce-accordion.js', array(), false, true );
				}
			}
