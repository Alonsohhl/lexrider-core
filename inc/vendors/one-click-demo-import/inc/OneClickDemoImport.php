...
//		add_action( 'all_admin_notices', array( $this, 'finish_notice_output_capturing' ), PHP_INT_MAX );
		...
		if ( $this->plugin_page === $hook || ( 'admin.php' === $hook && $this->plugin_page_setup['menu_slug'] === esc_attr( $_GET['import'] ) ) ) {
			...
			wp_localize_script( 'ocdi-main-js', 'ocdi',
				array(
					...
					'texts'            => array(
						'missing_preview_image'    => esc_html__( 'No preview image defined for this import.', 'lexrider-core' ),
						'dialog_title'             => esc_html__( 'Are you sure?', 'lexrider-core' ),
						'dialog_no'                => esc_html__( 'Cancel', 'lexrider-core' ),
						'dialog_yes'               => esc_html__( 'Yes, import!', 'lexrider-core' ),
						'selected_import_title'    => esc_html__( 'Selected demo import:', 'lexrider-core' ),
						'installing'               => esc_html__( 'Installing...', 'lexrider-core' ),
						'importing'                => esc_html__( 'Importing...', 'lexrider-core' ),
						'successful_import'        => esc_html__( 'Successfully Imported!', 'lexrider-core' ),
						'install_plugin'           => esc_html__( 'Install Plugin', 'lexrider-core' ),
						'installed'                => esc_html__( 'Installed', 'lexrider-core' ),
						'import_failed'            => esc_html__( 'Import Failed', 'lexrider-core' ),
						'import_failed_subtitle'   => esc_html__( 'Whoops, there was a problem importing your content.', 'lexrider-core' ),
						'plugin_install_failed'    => esc_html__( 'Looks like some of the plugins failed to install. Please try again. If this issue persists, please manually install the failing plugins and come back to this step to import the theme demo data.', 'lexrider-core' ),
						'content_filetype_warn'    => esc_html__( 'Invalid file type detected! Please select an XML file for the Content Import.', 'lexrider-core' ),
						'widgets_filetype_warn'    => esc_html__( 'Invalid file type detected! Please select a JSON or WIE file for the Widgets Import.', 'lexrider-core' ),
						'customizer_filetype_warn' => esc_html__( 'Invalid file type detected! Please select a DAT file for the Customizer Import.', 'lexrider-core' ),
						'redux_filetype_warn'      => esc_html__( 'Invalid file type detected! Please select a JSON file for the Redux Import.', 'lexrider-core' ),
					),
				)
			);
	...
	public function upload_manual_import_files_callback() {
		...
		if ( empty( $_FILES ) ) {
			wp_send_json_error( esc_html__( 'Manual import files are missing! Please select the import files and try again.', 'lexrider-core' ) );
		}
		...
		$this->import_files[ $this->selected_index ]['import_file_name'] = esc_html__( 'Manually uploaded files', 'lexrider-core' );
			...
			elseif ( ! empty( $this->import_files[ $this->selected_index ] ) ) { // Use predefined import files from wp filter: ocdi/import_files.
				...
				if ( is_wp_error( $this->selected_import_files ) ) {
					...
					Helpers::log_error_and_send_ajax_response(
						...
						$this->log_file_path,
						esc_html__( 'Downloaded files', 'lexrider-core' )
					);
				}
				...
				$log_added = Helpers::append_to_file(
					sprintf( /* translators: %s - the name of the selected import. */
						__( 'The import files for: %s were successfully downloaded!', 'lexrider-core' ),
				...
				wp_send_json( esc_html__( 'No import files specified!', 'lexrider-core' ) );
			}
		}
	...
	private function final_response() {
		...
		$response['title'] = esc_html__( 'Import Complete!', 'lexrider-core' );
		$response['subtitle'] = '<p>' . esc_html__( 'Congrats, your demo was imported successfully. You can now begin editing your site.', 'lexrider-core' ) . '</p>';
		$response['message'] = '<img class="ocdi-imported-content-imported ocdi-imported-content-imported--success" src="' . esc_url( OCDI_URL . 'assets/images/success.svg' ) . '" alt="' . esc_attr__( 'Successful Import', 'lexrider-core' ) . '">';

		if ( ! empty( $this->frontend_error_messages ) ) {
			$response['subtitle'] = '<p>' . esc_html__( 'Your import completed, but some things may not have imported properly.', 'lexrider-core' ) . '</p>';
			$response['subtitle'] .= sprintf(
				wp_kses(
				/* translators: %s - link to the log file. */
					__( '<p><a href="%s" target="_blank">View error log</a> for more information.</p>', 'lexrider-core' ),
					array(
						'p'      => [],
						'a'      => [
							'href'   => [],
