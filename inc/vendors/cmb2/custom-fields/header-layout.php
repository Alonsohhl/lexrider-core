...
class OSF_CMB2_Field_Header_Layout {
	...
	public function render( $field, $field_escaped_value, $field_object_id, $field_object_type, $field_type_object ) {
		...
		$option = '<option value="global" selected>' . esc_html__( 'Global', 'lexrider-core' ) . '</option>';
		$option .= '<option value="default" selected>' . esc_html__( 'Default', 'lexrider-core' ) . '</option>';
		if ( $headers ) {
