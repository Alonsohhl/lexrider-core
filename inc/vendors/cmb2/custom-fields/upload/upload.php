...
class OSF_Field_Opal_Upload {
	...
	public static function init() {
		...
		add_action( 'wp_ajax_nopriv_opalrealestate_delete_property_image', array(
			__CLASS__,
	...
	public static function upload_user_avatar() {
		...
		if ( ! wp_verify_nonce( $nonce, 'allow_uploaded' ) || ! $user_id ) {
			$ajax_response = array( 'success' => false, 'reason' => __( 'Security check failed!', 'lexrider-core' ) );
	...
	public static function setup_admin_scripts() {
		...
		$prop_data = array(
			...
			'fileTypeTitle' => esc_html__( 'Valid file formats', 'lexrider-core' ),
			'message'       => esc_html__( 'Please enter only digits', 'lexrider-core' ),
		);
	...
	public static function delete_property_image() {
		...
		if ( ! isset( $_POST['property_id'], $_POST['attachment_id'] ) ) {
			...
			$ouput->message = __( 'Could not delete this?', 'lexrider-core' );
		...
		if ( $user_id == $author_id ) {
			...
			$ouput->message = __( 'Deleted this done', 'lexrider-core' );
			$ouput->status  = true;
			echo json_encode( $ouput );
			exit;
		}
		...
		if ( ! isset( $property_images[ $attchment_id ] ) ) {
			$ouput->message = __( 'Could not found this in the collection?', 'lexrider-core' );
			echo json_encode( $ouput );
			exit;
		}
		if ( ! opalrealestate_is_own_property( $post_id, $user_id ) ) {
			$ouput->message = __( 'The post is not owned by this user?', 'lexrider-core' );
			echo json_encode( $ouput );
			exit;
		}
