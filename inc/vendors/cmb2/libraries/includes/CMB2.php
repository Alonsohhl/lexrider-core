...
class CMB2 extends CMB2_Base {
	...
	public function __construct( $config, $object_id = 0 ) {
		...
		if ( empty( $config['id'] ) ) {
			wp_die( esc_html__( 'Metabox configuration is required to have an ID parameter.', 'lexrider-core' ) );
		}
	...
	public function render_group_row( $field_group ) {
		...
		echo '
			<div class="cmbhandle" title="', esc_attr__( 'Click to toggle', 'lexrider-core' ), '"><br></div>
	...
	public function is_alternate_context_box() {
		return $this->prop( 'context' )
		       && in_array( $this->prop( 'context' ), array(
				'form_top',
				'before_permalink',
				'after_title',
				'after_editor'
