...
class CMB2_hookup extends CMB2_Hookup_Base {
	...
	public function term_hooks() {
		if ( ! function_exists( 'get_term_meta' ) ) {
			wp_die( esc_html__( 'Term Metadata is a WordPress 4.4+ feature. Please upgrade your WordPress install.', 'lexrider-core' ) );
		}

		if ( ! $this->cmb->prop( 'taxonomies' ) ) {
			wp_die( esc_html__( 'Term metaboxes configuration requires a "taxonomies" parameter.', 'lexrider-core' ) );
		}
	...
	public function context_box_title_markup_open( $add_handle = true ) {
		...
		if ( $add_handle ) {
			...
			echo '<span class="screen-reader-text">' . sprintf( __( 'Toggle panel: %s', 'lexrider-core' ), $title ) . '</span>';
	...
	public function add_metaboxes() {
		...
		foreach ( $this->cmb->box_types() as $object_type ) {
			...
			add_meta_box( $this->cmb->cmb_id, $this->cmb->prop( 'title' ), array(
				$this,
				'metabox_callback'
			), $object_type, $this->cmb->prop( 'context' ), $this->cmb->prop( 'priority' ) );
		}
	}
