...
class OSF_Scripts {
	...
	public function add_frontend_scripts() {
		wp_register_script( 'otf-countdown', trailingslashit( LEXRIDER_CORE_PLUGIN_URL ) . 'assets/js/countdown.js', array( 'jquery' ), false, true );
		...
		if ( get_theme_support( 'opal-customize-css' ) ) {
			...
			if ( is_customize_preview() || $dev_mode ) {
				wp_add_inline_style( 'lexrider-style', osf_theme_custom_css() );
			} else {
				...
				wp_add_inline_style( 'lexrider-style', $cssCustom );
			}

			if ( is_customize_preview() ) {
				add_action( 'wp_footer', array( $this, 'customizer_inline_css' ) );
			}
			wp_add_inline_style( 'lexrider-style', apply_filters( 'osf_theme_custom_inline_css', '' ) );
		}

		// Owl Carousel
		wp_enqueue_script( 'owl-carousel', trailingslashit( LEXRIDER_CORE_PLUGIN_URL ) . 'assets/js/libs/owl.carousel.js', array( 'jquery' ), '2.2.1' );
		wp_enqueue_script( 'osf-carousel', trailingslashit( LEXRIDER_CORE_PLUGIN_URL ) . 'assets/js/carousel.js', array( 'owl-carousel' ) );

		wp_enqueue_script( 'smoothscroll', trailingslashit( LEXRIDER_CORE_PLUGIN_URL ) . 'assets/js/SmoothScroll.min.js', array( 'jquery' ), '1.4.8', true );

		wp_enqueue_style('lexrider-update',trailingslashit( LEXRIDER_CORE_PLUGIN_URL ) . 'assets/css/lexrider-update.css');
	}
	...
	public function init_panel_customize_js() {
		wp_enqueue_script(
			'osf-admin-customize',
			LEXRIDER_CORE_PLUGIN_URL . 'assets/js/customizer/customize.js',
			array( 'jquery' ),
			null,
			true
		);
	...
	public function init_preview_js() {
		wp_enqueue_style( 'osf-theme-preview', LEXRIDER_CORE_PLUGIN_URL . 'assets/css/customize-preview.css' );
		wp_enqueue_script( 'osf-theme-preview', LEXRIDER_CORE_PLUGIN_URL . 'assets/js/customizer/preview.js', array(
			'jquery',
			'customize-preview'
		), LEXRIDER_CORE_VERSION, true );
	...
	public function add_admin_scripts( $hook ) {
		...
		if ( $hook === 'widgets.php' ) {
			wp_enqueue_script(
				'osf-admin-color',
				LEXRIDER_CORE_PLUGIN_URL . 'assets/3rd-party/alpha-color-picker/alpha-color-picker.js',
				array( 'jquery' ),
				null,
				true
			);
		...
		wp_enqueue_script(
			'osf-admin-select2',
			LEXRIDER_CORE_PLUGIN_URL . 'assets/3rd-party/select2/select2.js',
			array( 'jquery' ),
			null,
			true
		);
...
//		wp_enqueue_style( 'opal-icon', get_theme_file_uri( 'assets/css/opal-icon.css' ) );
		...
		wp_enqueue_style( 'osf-admin-style',
			LEXRIDER_CORE_PLUGIN_URL . 'assets/css/admin.css' );

		if ( $hook == 'post-new.php' || $hook == 'post.php' ) {
			wp_enqueue_script( 'osf-admin-page', trailingslashit( LEXRIDER_CORE_PLUGIN_URL ) . 'assets/js/admin-page.js' );
		}
