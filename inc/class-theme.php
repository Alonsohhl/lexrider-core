...
class OSF_Theme_Setup {
	...
	public function widgets_init() {
		...
		register_sidebar( array(
			'name'          => esc_html__( 'Single Posts Sidebar', 'lexrider-core' ),
			'id'            => 'sidebar-single-post',
			'description'   => esc_html__( 'Add widgets here to appear in your sidebar on single posts.', 'lexrider-core' ),
		...
		register_sidebar( array(
			'name'          => esc_html__( 'Blog Sidebar', 'lexrider-core' ),
			'id'            => 'sidebar-blog',
			'description'   => esc_html__( 'Add widgets here to appear in your sidebar on blog posts and archive pages.', 'lexrider-core' ),
			'before_widget' => '<section id="%1$s" class="widget %2$s">',
			'after_widget'  => '</section>',
			'before_title'  => '<h2 class="widget-title">',
			'after_title'   => '</h2>',
		...
		register_sidebar( array(
			'name'          => esc_html__( 'Page Sidebar', 'lexrider-core' ),
			'id'            => 'sidebar-page',
			'description'   => esc_html__( 'Add widgets here to appear in your pages.', 'lexrider-core' ),
			'before_widget' => '<section id="%1$s" class="widget %2$s">',
			'after_widget'  => '</section>',
			'before_title'  => '<h2 class="widget-title">',
			'after_title'   => '</h2>',
	...
	public function modal_login_register() {
		include_once trailingslashit( LEXRIDER_CORE_PLUGIN_DIR ) . 'templates/modal/login-register-form.php';
	}
	...
	public function add_body_class( $classes ) {
		...
		if ( osf_is_blog_archive() ) {
			...
			$classes[] = 'opal-content-layout-2cr';
		} elseif (is_singular( 'osf_report' ) && is_active_sidebar( 'sidebar-single-post' )) {
            $classes[] = 'opal-content-layout-2cr';
        }
	...
	private function set_main_nav_content( $items ) {
		...
		$items .= '<a data-search-toggle="toggle" data-target=".' . $id . '" class="fa fa-search" title="' . __( "search", "lexrider-core" ) . '"></a>';
	...
	public function init_sidebar( $sidebar ) {
		if ( (is_singular( 'post' ) && is_active_sidebar( 'sidebar-single-post')) || (is_singular( 'osf_report' ) && is_active_sidebar( 'sidebar-single-post'))  ) {
...
HTML;
	...
	public function render_related_posts() {
		if ( is_active_sidebar( 'sidebar-single-post' ) || ( ! class_exists( 'LexriderCore' ) && is_active_sidebar( 'sidebar-blog' ) ) ) {
			lexrider_fnc_related_post( 2 );
		} else {
			lexrider_fnc_related_post( 3 );
		}
	}
