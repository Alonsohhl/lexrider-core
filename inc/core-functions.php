...
function osf_get_google_fonts() {
	$content = file_get_contents( LEXRIDER_CORE_PLUGIN_DIR . 'webfonts.json' );
...
if ( ! function_exists( 'osf_scrape_instagram' ) ) {
	...
	function osf_scrape_instagram( $username, $slice = 9 ) {
		...
		if ( false === ( $instagram = get_transient( 'otf-instagram-media-new-' . sanitize_title_with_dashes( $username ) ) ) ) {
			...
			if ( ! $insta_array ) {
				return new WP_Error( 'bad_json', esc_html__( 'Instagram has returned invalid data.', 'lexrider-core' ) );
			}
			...
			if ( isset( $insta_array['entry_data']['UserProfile'][0]['userMedia'] ) ) {
				...
				return new WP_Error( 'bad_json_2', esc_html__( 'Instagram has returned invalid data.', 'lexrider-core' ) );
			}


			if ( ! is_array( $images ) ) {
				return new WP_Error( 'bad_array', esc_html__( 'Instagram has returned invalid data.', 'lexrider-core' ) );
			}
			...
			switch ( $type ) {
				...
				case 'old_2':
					foreach ( $images as $image ) {
						...
						$caption = esc_html__( 'Instagram Image', 'lexrider-core' );
		...
		if ( ! empty( $instagram ) ) {
			...
			return new WP_Error( 'no_images', esc_html__( 'Instagram did not return any images.', 'lexrider-core' ) );
		}
	}
}
...
function sanitize_font_weight($arr) {
	if(is_array($arr) && !empty($arr['fontWeight'])){
	    $weight = preg_replace('/\D+/m', '', $arr['fontWeight']);
		$arr['fontWeight'] = $weight ? $weight : '400';
	}
	return $arr;
}

if (!function_exists('lexrider_get_page_by_title')) {
    function lexrider_get_page_by_title($title,$output = OBJECT, $post_type = 'page') {
        $query = new WP_Query(
            [
                'post_type' => $post_type,
                'title'     => $title,
